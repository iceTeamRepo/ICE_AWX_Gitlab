...
#-----------------------------------------------------------------------------------------------------------------------------------
# generate_pg_password_md5 :  redis 구성에 사용할 password hash generator
#-----------------------------------------------------------------------------------------------------------------------------------
generate_pg_password_md5() {
  ...
  case "$service" in
    gitlab|redis|gitlab_replicator|gitlab-consul)
      echo -e "$password\n$password" | sudo gitlab-ctl pg-password-md5 "$service"
      ;;
    *)
      echo "Usage: generate_pg_password_md5 {gitlab|redis|gitlab_replicator|gitlab-consul} <password>"
      return 1
      ;;
  esac
}
...
EOF
  ...
  install_log "configure_redis"
}


#-----------------------------------------------------------------------------------------------------------------------------------
# configure_gitlab_reload : redis  재시작
#-----------------------------------------------------------------------------------------------------------------------------------
